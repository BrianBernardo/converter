...
#Windows + Widget for 5 bit number to 1's and 2's complement

def B_12():
    def b_1_2():
        output_text_widget.delete('1.0', END)
        output_text_widget2.delete('1.0', END)
        ones_complement = ones_comp(input_text_widget.get())
        twos_complement = twos_comp(ones_complement)
        output_text_widget.insert(END, ones_complement)
        output_text_widget2.insert(END, twos_complement)

    D2Hwin = Tk()
    D2Hwin.title("5 bit number to 1's and 2's complement")
    ...
    l0 = Label(f1, font=('arial', 25, 'bold'), text='Input:', bg=col)
    input_text_widget = Entry(f2)
    execute_widget = Button(f2, text='    Convert   ', font=('arial', 15, 'bold'), command=b_1_2)
    l1 = Label(f3, font=('arial', 25, 'bold'), text="1's complement", bg=col)
    output_text_widget = Text(f4, width=45, height=1)
    l2 = Label(f5, font=('arial', 25, 'bold'), text="2's complement", bg=col)
    output_text_widget2 = Text(f6, width=45, height=1)
    ...
    output_text_widget2.pack(side=LEFT, fill=BOTH, padx=10, pady=10)
...
#Function for getting the 1's and 2's complement of a 5 bit number


def ones_comp(num):
    if len(num) == 5 and all(bit in '01' for bit in num):
        return ''.join('1' if bit == '0' else '0' for bit in num)
    return "invalid input"
def twos_comp(ones_complement):
    twos = bin(int(ones_complement, 2)+1)[2:]
    if len(twos) > 5:
        twos = twos[-5:]
    else:
        twos = twos.zfill(5)
...
submenu.add_separator()
submenu.add_cascade(label="5 binary to 1's and 2's complement",command=B_12)
submenu.add_separator()
